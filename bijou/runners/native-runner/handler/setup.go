...
import (
	...
	"github.com/wings-software/dlite/bijou/runners/native-runner/engine"
	"github.com/wings-software/dlite/bijou/runners/native-runner/engine/docker"
	...
	"github.com/wings-software/dlite/bijou/runners/native-runner/livelog"
	"github.com/wings-software/dlite/bijou/runners/native-runner/logger"
	"github.com/wings-software/dlite/bijou/runners/native-runner/pipeline"
	prruntime "github.com/wings-software/dlite/bijou/runners/native-runner/pipeline/runtime"
	"github.com/wings-software/dlite/bijou/runners/native-runner/ti"
...
func HandleSetupInternal(ctx context.Context, s api.SetupRequest) error {
	st := time.Now()
	id := s.ID
	updateVolumes(s)
	...
	if err != nil {
		logrus.WithError(err).Errorln("could not instantiate engine for the execution")
		// WriteError(w, err)
		return err
	}
	...
	logrus.Traceln("starting the setup process")
	...
	if err := ex.Add(id, stageData); err != nil {
		logrus.WithError(err).Errorln("could not store stage data")
		//WriteError(w, err)
		return err
	}

	if err := engine.Setup(ctx, cfg); err != nil {
	...
	logrus.Traceln("completed the setup process")
...
func HandleSetup(config *config.Config) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		HandleSetupInternal(r.Context(), s)
