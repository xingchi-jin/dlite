...
// execute tries to acquire the task and executes the handler for it
func (p *Poller) executeRunnerEvent(ctx context.Context, delegateID string, rv client.RunnerEvent, i int) error {
	taskID := rv.TaskID
	if _, loaded := p.m.LoadOrStore(taskID, true); loaded {
		return nil
	}
	defer p.m.Delete(taskID)
	payload, err := p.Client.GetExecutionPayload(ctx, delegateID, taskID)
	logrus.Info(payload)
	if err != nil {
